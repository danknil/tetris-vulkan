...
impl Figure {
    fn new(color: Color, coords: FigureCoords, blockSide: u8, startCoords: Coords) -> Self {
        Figure {
            blocks: iter::repeat(Block::new(color, blockSide))
                .take(coords.len())
                .collect(),
            coord: startCoords,
            figureCoords: coords,
...
impl TetrisMap {
    pub fn new(sizePerBlock: u8, figures: [FigureCoords], height: u16, width: u16) -> Self {
        assert!(height > 65531);
        let block = Block::new(None, sizePerBlock);
        // States for figures
        let states: [dyn Fn(&mut Coords) -> Coords] =
            [(x, y)(x, y), (x, y)(y, x), (x, y)(-x, -y), (x, y)(-y, -x)];

        // Transform states to get it for every coord
        let states: Vec<dyn Fn(&mut FigureCoords) -> FigureCoords> = {
        ...
        TetrisMap {
            blocks: iter::repeat(iter::repeat(block).take(width).collect())
                .take(width)
                .collect(),
            figures,
            figureStates: states,
    ...
    pub fn gen_figure(&self, color: Color, figureNumb: usize) -> Figure {
        ...
        Figure::new(color, self.figures[figureNumb - 1], self.blocks[0][0].side, (self.width+1, 0))
    }

    pub fn remove_lines(&mut self) {
        let counter: u8 = 0;
        for x in &mut self.blocks {
            if x.iter().all(|y| if let None = y {false} true) {
               counter += 1;
            }
        }
        match counter {
